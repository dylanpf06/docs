...
- Next, you will need to create an account on DS Cloud, which can be done through the [login page](https://dscloud.app/login). You can use either your email or Discord, or both, to create an account.
- Finally, you need to have an active paid plan on your account. You can view our plans and purchase one according to your needs [here](https://dscloud.app/plans).
...
>
  The dscloud.app file is a configuration file that will be used to
...
<Tabs>
  <Tab title="Javascript">
    - If the `START` field is not defined in the configuration file, DS Cloud will by default run `node` with optimization options and specific parameters. If the `START` field is defined in the configuration file, the value in the `START` field will be executed directly. For more information on configuration file parameters, visit [configuration parameters](https://docs.dscloud.app/getting-started/config-file#configuration-parameters).
  </Tab>
  <Tab title="Typescript">
    - If the `START` field is not defined in the configuration file, DS Cloud will by default run `npx tsx MAIN` to compile and execute the main TypeScript file specified by `MAIN` field. If the `START` field is defined in the configuration file, the value in the `START` field will be executed directly. For more information on configuration file parameters, visit [configuration parameters](https://docs.dscloud.app/getting-started/config-file#configuration-parameters).
  </Tab>
</Tabs>

- If the `node_modules` folder does not exist, DS Cloud will run `npm install --no-package-lock --no-audit --no-fund` to install dependencies and clean the `.npm` folder.

## 🖥️ Preparing Your Node.js Project

### 📁 Essential Configuration Files

When preparing your Node.js project for deployment on DS Cloud, ensure that you include your configuration file (`dscloud.app/.config`) and the essential files of your application in a zip file.

<Warning>The configuration (`dscloud.app/.config`) file must be located in the root of the zip file. Otherwise, DS Cloud will not be able to find it during deployment.</Warning>
...
By doing this, you can ensure that your Node.js application is optimally prepared and secure for deployment to DS Cloud.
...
<Note>
  DS Cloud natively supports `TypeScript`, running your project through
...
>
  ...
  <Accordion title="Bad Memory" icon="memory" iconType="solid">
    ...
    avoid the
    [LACK_OF_RAM](https://docs.dscloud.app/faqs/troubleshooting/LACK_OF_RAM)
    error.
  </Accordion>
</AccordionGroup>
...
After preparing your project files, you can now upload them to DS Cloud and host your project.

<Tabs>
  <Tab title="Dashboard Upload">
    Access the [DS Cloud
    Dashboard](https://dscloud.app/dashboard/upload) and upload your project
    files.
    <Frame>
  ...
  <Tab title="CLI Upload">
    <Steps>
      ...
      <Step title="Second Step">
        ...
        authorization key [here](https://dscloud.app/dashboard/settings) by clicking on
      ...
      <Step title="Third Step">
        Finally, to deploy your application to DS Cloud using the CLI, you
